...
When the `CModal` is open, it's rendered within a portal and all it's siblings have
...
| useInert             | `boolean`                                     | `true`    | A11y: If `true`, all elements behind the `Modal` will have `aria-hidden` set to `true` so that screen readers can only see the `Modal`. |
